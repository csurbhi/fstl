 ...
 * most of any other 'n'
 */
static struct rb_node * select_zone_to_clean(struct ctx *ctx, int mode)
{
	int zonenr = -2;
...
complete:
	...
	kmem_cache_free(ctx->gc_rb_node_cache, container_of(node, struct gc_rb_node, rb));
