 ...
 */
static struct extent *_lsdm_rb_geq(struct rb_root *root, sector_t lba, int print)
{
	...
	while (node) {
		...
		//printk(KERN_ERR "\n %s Searching lba: %llu, found:  (e->lba: %llu, e->pba: %llu e->len: %llu) \n", __func__, lba, e->lba, e->pba, e->len); 
		...
		if (e->lba > lba) {
			...
			//	printk(KERN_ERR "\n \t\t Going right! ");
				...
				//printk(KERN_ERR "\n %s Found an overlapping e, returning e \n", __func__);
				return e;
			}
		}
	}
	//printk(KERN_ERR "\n did not find a natural match and so returning the next higher: %s  \n", (higher == NULL) ? "null" : "not null");
	return higher;
}

static struct extent *lsdm_rb_geq(struct ctx *ctx, sector_t lba, int print)
{
	struct extent *e = NULL;
	...
	if (prev) {
		...
		*/
		if((prev->lba + prev->len) == e->lba) {
			if ((prev->pba + prev->len) == e->pba) {
				prev->len += e->len;
				//printk(KERN_ERR "\n Merged with prev ! e->lba: %llu e->pba: %llu e->len: %llu, new len: %llu", e->lba, e->pba, e->len, prev->len);
...
static struct extent * lsdm_rb_insert(struct ctx *ctx, struct extent *new)
{
	...
	if (ret < 0) {
		printk(KERN_ERR"\n !!!! Corruption while Inserting: lba: %llu pba: %llu len: %llu", new->lba, new->pba, new->len);
		BUG();
	}
#endif
	return NULL;
...
static void find_and_remove_rev_tm(struct ctx *ctx, sector_t lba, unsigned int len)
{
	...
	//e = lsdm_rb_geq(ctx, lba, 1);
	while(1) {
		e = _lsdm_rb_geq(&ctx->extent_tbl_root, lba, 0);
		/* case of no overlap */
		if ((e == NULL) || (e->lba >= (lba + len))) { // This will never happen: (e->lba + e->len) <= lba))
			break;
		}
		...
		if (pba > ctx->sb->max_pba) {
			printk(KERN_ERR "\n %s %d pba: %llu e->pba: %llu diff: %d  \n", __func__, __LINE__, pba, e->pba, diff);
			dump_stack();
			BUG();
		}
		if (overlap >= len) { 
...
static int select_zone_to_clean(struct ctx *ctx, int mode, const char *func)
{
	...
	//
		...
		list_for_each_entry(znode, &cnode->znodes_list, list) {
			//printk(KERN_ERR "\n %s zone: %u has %d blks caller: %s", __func__, znode->zonenr, znode->vblks, func);
			//printk(KERN_ERR "\n 2. %s ctx->nr_gc_nodes: %d ", __func__, ctx->nr_gc_nodes);
			return znode->zonenr;
		}

	//}
	//printk(KERN_ERR "\n %s ctx->nr_gc_nodes: %d ", __func__, ctx->nr_gc_nodes);
	/* TODO: Mode: FG_GC */
	return -1;
}
...
int get_sit_ent_vblocks(struct ctx *ctx, int zonenr, int free);
...
static int write_valid_gc_extents(struct ctx *ctx, int zonenr)
{
	...
	vblks = get_sit_ent_vblocks(ctx, zonenr, 0);
...
{	
	 ...
	 * 1 to set it
	 */
	ctx->freezone_bitmap[bytenr] = bitmap[bytenr] | (1 << bitnr);
	ctx->nr_freezones = ctx->nr_freezones + 1;
	//get_byte_string( ctx->freezone_bitmap[bytenr], str);
...
again:
	...
	if (zone_nr < 0) {
		...
		printk(KERN_ERR "%s Number of free zones available: %d", __func__, ctx->nr_freezones);
		if (trial < 2)  {
			timeout = jiffies + 100;
	...
	if(ctx->warm_gc_wf_end > ctx->sb->max_pba) {
...
void md_endio(struct bio * bio)
{
	if (bio->bi_status != BLK_STS_OK) {
		...
		//panic("Could not write the some metadata page");
	}
...
void move_write_frontier(struct ctx *ctx, sector_t s8)
{
	...
	if (ctx->free_sectors_in_wf < NR_SECTORS_IN_BLK) {
		...
		//trace_printk("\n %s zone: %d #valid blks: %d ", __func__, zonenr, get_sit_ent_vblocks(ctx, zonenr, 0));
		//mutex_lock(&ctx->bm_lock);
		ret = get_new_zone(ctx);
		//mutex_unlock(&ctx->bm_lock);
		if (ret) {
...
void print_zones_vblocks(struct ctx *ctx) 
{
	...
	for (i=0; i<ctx->sb->zone_count_main; i++) {
		vblocks = get_sit_ent_vblocks(ctx, i, 1);
		if (!vblocks)
...
void sit_ent_vblocks_decr(struct ctx *ctx, sector_t pba)
{
	...
	ptr = ptr + index;
	if (!ptr->vblocks) {
		printk("\n (%s) pba: %llu segnr: %llu vblocks: %u mtime:%lu \n", __func__, pba, zonenr, ptr->vblocks, ptr->mtime);
	}
	BUG_ON(!ptr->vblocks);
	...
	/*
	if (!ptr->vblocks) {
		printk(KERN_ERR "\n %s done! zone: %llu vblocks: %d pba: %llu, index: %d , ptr: %p", __func__, zonenr, ptr->vblocks, pba, index, ptr);
		printk(KERN_ERR "\n ***** zone: %llu hot_zone: %d, warm_gc_zone: %d ", zonenr, get_zone_nr(ctx, ctx->ckpt->hot_frontier_pba), get_zone_nr(ctx, ctx->ckpt->warm_gc_frontier_pba));
	} */
	if ((zonenr != get_zone_nr(ctx, ctx->ckpt->hot_frontier_pba)) &&
		    (zonenr != get_zone_nr(ctx, ctx->ckpt->warm_gc_frontier_pba))) {
		/* add mtime here */
...
int remove_zone_from_gc_tree(struct ctx *ctx, unsigned int zonenr)
{
	...
	while (link) {
		...
		if (znode->zonenr == zonenr) {
			...
			ctx->nr_gc_nodes -= 1;
			//printk(KERN_ERR "\n %s removing zone: %u from the gc_cost_root tree, ctx->nr_gc_node: %d", __func__, zonenr, ctx->nr_gc_nodes);
			return (0);
		}
		if (znode->zonenr < zonenr) {
			link = link->rb_right;
	...
	/* did not find the zone in rb tree, we should not be here ideally */
	return (-1);
}
...
struct gc_zone_node * add_zonenr_gc_zone_tree(struct ctx *ctx, unsigned int zonenr, u32 nrblks)
{
	...
	//printk(KERN_ERR "\n %s Added zone: %u nrblks: %d to gc tree! nr_gc_nodes: %d \n", __func__, zonenr, nrblks, ctx->nr_gc_nodes);
	return znew;
}
...
int update_gc_tree(struct ctx *ctx, unsigned int zonenr, u32 nrblks, u64 mtime, const char * caller)
{
	...
	if (znode->ptr_to_cost_node) {
		 ...
		 * then we have to remove the cost node from the tree
		 */
		//printk(KERN_ERR "\n %s zonenr: %u vblks: %u cost_node:%p ", __func__, zonenr, nrblks, new);
	...
	//printk(KERN_ERR "\n Allocate gc_cost_node: %p, cost: %u zone: %u nrblks: %u list_head: %p \n", new, cost, znode->zonenr, znode->vblks, &new->znodes_list);
	new->cost = cost;
	RB_CLEAR_NODE(&new->rb);
	...
	//printk(KERN_ERR "\n %s zone to clean: %u ", __func__, zonenr);
	return 0;
}
...
int read_metadata(struct ctx * ctx)
{
	...
	if (sb2->zone_count_main % BITS_IN_BYTE) {
		...
		ctx->bitmap_bit = ctx->bitmap_bit - 1;
	}
