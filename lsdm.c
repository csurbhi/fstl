	...
	if (!ptr->vblocks) {
		...
		if (!ret) {
			mark_zone_free(ctx , zonenr);
		       ...
		       /* This zone cannot be reused, hence we do not
			* mark it free 
			*/
		}
...
void remove_tm_blk_kv_store(struct work_struct *w)
{
	struct ctx *ctx;
	struct page *page;
	struct tm_page_write_ctx *tm_page_write_ctx = container_of(w, struct tm_page_write_ctx, work);
	struct tm_page *tm_page = tm_page_write_ctx->tm_page;

	//printk(KERN_ERR "\n %s in progress!", __func__);

	if (!tm_page) {
		panic("tm_page cannot be null now! ");
		return;
	}
	page = tm_page->page;
	if (!page) {
 ...
 * flushed to disk. Currently, we also remove every page after it has
 * been flushed!
 */
...
int lsdm_write_io(struct ctx *ctx, struct bio *bio)
{
	...
	do {
		...
		lsdm_update_range(ctx, &ctx->extent_tbl_root, lba, wf, s8);
		lsdm_update_range(ctx, &ctx->rev_tbl_root, wf, lba, s8);
		/*-------------------------------*/
		up_write(&ctx->metadata_update_lock);

		//printk(KERN_ERR "\n %s Submitted bio, lba: %llu, pba: %llu", __func__, lba, wf);
	} while (split != clone);
...
int read_translation_map(struct ctx *ctx)
{
	...
	while(i < nrblks) {
		...
		read_extents_from_block(ctx, tm_entry, nr_extents_in_blk, pba);
		i = i + 1;
		__free_pages(page, 0);
		pba = pba + 1;
	}
...
int read_seg_info_table(struct ctx *ctx)
{
	...
	pba = 0;
	printk(KERN_ERR "\n %s Read seginfo from pba: %llu", __func__, sb->sit_pba);
